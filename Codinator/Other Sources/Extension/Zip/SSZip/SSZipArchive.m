...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination delegate:(id<SSZipArchiveDelegate>)delegate withError:(NSError *)error{
    return [self unzipFileAtPath:path toDestination:destination overwrite:YES password:nil error:&error delegate:delegate progressHandler:nil completionHandler:nil];
}
    ...
    do {
        @autoreleasepool {
            ...
            if (!fileIsSymbolicLink) {
                ...
                while (fp) {
                    NSInteger readBytes = unzReadCurrentFile(zip, buffer, 4096);
            ...
            else
            {
                ...
                {
                    buffer[bytesRead] = (NSInteger)0;
                    [destinationPath appendString:[NSString stringWithUTF8String:(const char*)buffer]];
                }
                
                // Create the symbolic link (making sure it stays relative if it was relative before)
                NSInteger symlinkError = symlink([destinationPath cStringUsingEncoding:NSUTF8StringEncoding],
...
+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray *)paths delegate:(id<SSZipArchiveDelegate>)delegate
{
    BOOL success = NO;
    SSZipArchive *zipArchive = [[SSZipArchive alloc] initWithPath:path];
    if ([zipArchive open]) {
    ...
    // Message delegate
    if (success && [delegate respondsToSelector:@selector(zipArchiveDidZippedArchiveToPath:)]) {
        [delegate zipArchiveDidZippedArchiveToPath:path];
    }
...
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath delegate:(id<SSZipArchiveDelegate>)delegate{
    BOOL successL = [self createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:NO];
    
    // Message delegate
    if (successL && [delegate respondsToSelector:@selector(zipArchiveDidZippedArchiveToPath:)]) {
        [delegate zipArchiveDidZippedArchiveToPath:path];
...
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory delegate:(id<SSZipArchiveDelegate>)delegate {
    BOOL success = NO;
