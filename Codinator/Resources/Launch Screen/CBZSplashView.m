...
+ (instancetype)splashViewWithIcon:(UIImage *)icon backgroundColor:(UIColor *)backgroundColor frame:(CGRect)frame
{
  /* This component is useless without an icon */
  //NSParameterAssert(icon);
  
  return [[CBZRasterSplashView alloc] initWithIconImage:icon backgroundColor:backgroundColor frame:frame];
}
...
- (CGSize)iconStartSize
{
  if (!_iconStartSize.height) {
    ...
    _iconStartSize = CGSizeMake(200, 200);
